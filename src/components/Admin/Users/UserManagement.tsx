    ...
    <div className="space-y-6">
                ...
                <tr key={user.id} className="hover:bg-white/5 transition-colors">
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#0C3D06] border border-[#1AFF00]/40 flex items-center justify-center text-[#1AFF00] font-bold text-xs">
                        {user.name.charAt(0)}
                      </div>
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold ${
                        user.role === 'Super Admin'
                          ? 'bg-[#1AFF00]/20 text-[#1AFF00] border border-[#1AFF00]/40'
                          : user.role === 'Admin'
                  ...
                  <td className="py-3.5 px-4">
                    <span
                      className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold ${
                        user.status === 'Active'
                          ? 'bg-emerald-950/80 text-[#1AFF00] border border-[#1AFF00]/30'
                          : 'bg-red-950/80 text-red-400 border border-red-500/30'
                      }`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${
                          user.status === 'Active' ? 'bg-[#1AFF00]' : 'bg-red-400'
                        }`}
                      />
                      {user.status}
                    </span>
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-white/10 text-gray-300 hover:text-[#1AFF00]"
                        title={user.status === 'Active' ? 'Deactivate Account' : 'Activate Account'}
                      >
                        {user.status === 'Active' ? (
                          <UserX className="w-3.5 h-3.5 text-amber-400" />
                        ) : (
                          <UserCheck className="w-3.5 h-3.5 text-[#1AFF00]" />
                        )}
                      </button>
                      <button
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div className="flex items-center gap-2">
                <Shield className="w-5 h-5 text-[#1AFF00]" />
      ...
      {selectedUserForDrawer && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex justify-end">
          <div className="w-full max-w-md bg-[#080B0A] border-l border-white/10 h-full p-6 space-y-5 flex flex-col justify-between shadow-2xl animate-in slide-in-from-right duration-300">
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#0C3D06] border border-[#1AFF00]/40 flex items-center justify-center text-[#1AFF00] font-bold text-sm">
              ...
              <div className="p-4 rounded-xl bg-[#111615] border border-white/10 space-y-2 text-xs">
                <div className="flex justify-between">
                  <span className="text-gray-400">Current Role</span>
                  <span className="font-bold text-[#1AFF00]">{selectedUserForDrawer.role}</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="space-y-2 text-xs">
                ...
                {['knowledge_edit', 'ocr_approval', 'dictionary_review', 'user_management'].map((perm) => (
                  <div key={perm} className="p-2.5 rounded-lg bg-black/40 border border-white/5 flex items-center justify-between">
                    ...
                    <CheckCircle2 className="w-4 h-4 text-[#1AFF00]" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <button
              onClick={() => setSelectedUserForDrawer(null)}
              className="w-full py-2.5 rounded-xl bg-[#0C3D06] hover:bg-[#16e000] text-[#1AFF00] hover:text-black border border-[#1AFF00]/30 font-bold text-xs transition-colors"
            >
